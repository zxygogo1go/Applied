...
class EdgeLoss(nn.Module):
    ...
    def __init__(self, lossfn_type='l1', eps=1e-6):
        ...
        self.eps = eps
        self.lossfn = nn.MSELoss() if lossfn_type == 'l2' else nn.L1Loss()
    ...
    def forward(self, x, y):
        return self.lossfn(self.gradient(x), self.gradient(y))


class SAMLoss(nn.Module):
    """Spectral angle mapper loss for channel consistency."""
    ...
    def forward(self, x, y):
        ...
        denominator = torch.norm(x_flat, dim=1) * torch.norm(y_flat, dim=1)
        cosine = numerator / (denominator + self.eps)
