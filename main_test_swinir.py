...
def main():
    ...
    parser.add_argument('--rs_module', action='store_true', help='enable remote-sensing frequency refinement module')
    parser.add_argument('--rs_module_scale', type=float, default=0.1, help='residual scale of remote-sensing refinement')
    parser.add_argument('--rs_module_hidden', type=int, default=16, help='hidden channels of remote-sensing refinement')
    parser.add_argument('--non_strict_load', action='store_true', help='allow missing keys when loading older SwinIR weights')
...
def define_model(args):
    rs_args = dict(rs_module=getattr(args, 'rs_module', False),
                   rs_module_scale=getattr(args, 'rs_module_scale', 0.1),
                   rs_module_hidden=getattr(args, 'rs_module_hidden', 16))

    # 001 classical image sr
    if args.task == 'classical_sr':
        model = net(upscale=args.scale, in_chans=3, img_size=args.training_patch_size, window_size=8,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='pixelshuffle', resi_connection='1conv', **rs_args)
        param_key_g = 'params'
    ...
    # use 'pixelshuffledirect' to save parameters
        ...
        model = net(upscale=args.scale, in_chans=3, img_size=64, window_size=8,
                    ...
                    mlp_ratio=2, upsampler='pixelshuffledirect', resi_connection='1conv', **rs_args)
        param_key_g = 'params'
    ...
    # 003 real-world image sr
        ...
        if not args.large_model:
            ...
            model = net(upscale=args.scale, in_chans=3, img_size=64, window_size=8,
                        img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                        mlp_ratio=2, upsampler='nearest+conv', resi_connection='1conv', **rs_args)
        else:
            # larger model size; use '3conv' to save parameters and memory; use ema for GAN training
            model = net(upscale=args.scale, in_chans=3, img_size=64, window_size=8,
                        img_range=1., depths=[6, 6, 6, 6, 6, 6, 6, 6, 6], embed_dim=240,
                        num_heads=[8, 8, 8, 8, 8, 8, 8, 8, 8],
                        mlp_ratio=2, upsampler='nearest+conv', resi_connection='3conv', **rs_args)
    ...
    # 004 grayscale image denoising
        ...
        model = net(upscale=1, in_chans=1, img_size=128, window_size=8,
                    img_range=1., depths=[6, 6, 6, 6, 6, 6], embed_dim=180, num_heads=[6, 6, 6, 6, 6, 6],
                    mlp_ratio=2, upsampler='', resi_connection='1conv', **rs_args)
        param_key_g = 'params'
    ...
    if getattr(args, 'non_strict_load', False):
